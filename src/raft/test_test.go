...
func TestReElection2A(t *testing.T) {
	...
	DPrintf(1, "-----begin check one leader after halt %d -----", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader
	DPrintf(1, "-----begin check one leader after rejoin %d -----", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected
	DPrintf(1, "-----begin check no leader, halt %d, %d -----", leader2, (leader2+1)%servers)
	...
	DPrintf(1, "-----begin check one leader-----")
...
func TestFailAgree2B(t *testing.T) {
	...
	DPrintf(2, "-----disconnected %d-----", (leader+1)%servers)
	...
	DPrintf(2, "-----reconnected %d-----", (leader+1)%servers)
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	DPrintf(1, "-----disconnected %d-----", leader1)
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	DPrintf(1, "-----disconnected %d-----", leader2)

	// old leader connected again
	cfg.connect(leader1)
	DPrintf(1, "-----connected %d-----", leader1)
...
func TestBackup2B(t *testing.T) {
	...
	DPrintf(1, "-----remain %d %d-----", leader1, (leader1+1)%servers)
	tmpV := 50 // 50
	...
	DPrintf(1, "-----remain %d %d %d-----", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	DPrintf(1, "-----remain %d %d %d except %d-----",
		(leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers, other)
	...
	cfg.connect(other)
	DPrintf(1, "-----remain %d %d %d-----", (leader1+0)%servers, (leader1+1)%servers, other)
...
func TestPersist22C(t *testing.T) {
	...
	for iters := 0; iters < 5; iters++ {
		...
		cfg.disconnect((leader1 + 1) % servers)
		cfg.disconnect((leader1 + 2) % servers)
		DPrintf(1, "-----remain %d %d %d-----", (leader1+0)%servers, (leader1+2)%servers, (leader1+3)%servers)
		...
		cfg.connect((leader1 + 1) % servers)
		cfg.connect((leader1 + 2) % servers)
		DPrintf(1, "-----restart %d %d -----", (leader1+1)%servers, (leader1+2)%servers)

		time.Sleep(RaftElectionTimeout)

		cfg.start1((leader1 + 3) % servers)
		cfg.connect((leader1 + 3) % servers)
		DPrintf(1, "-----restart %d  remian %d %d %d-----",
			(leader1+3)%servers, (leader1+1)%servers, (leader1+2)%servers, (leader1+3)%servers)
